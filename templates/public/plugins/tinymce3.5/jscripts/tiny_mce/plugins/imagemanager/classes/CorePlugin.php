...
class Moxiecode_CorePlugin extends Moxiecode_ManagerPlugin {
	...
	function onInit(&$man) {
		...
		$man
				->registerFileSystem('file',
						isset($config['filesystem']) ? $config['filesystem']
								: 'Moxiecode_LocalFileImpl');
	...
	function onStream(&$man, $cmd, $input) {
		...
		if ($cmd == "streamFile") {
			...
			if (!$file->exists()) {
				...
				if (getClassName($file) == 'moxiecode_localfileimpl') {
					// Redirect to data
					$url = $man
							->removeTrailingSlash($config['preview.urlprefix'])
							. $man
									->convertPathToURI(
											$file->getParent() . "/"
													. str_replace("+", "%20",
															urlencode(
																	$file
																			->getName())),
											$config["preview.wwwroot"])
							. $config['preview.urlsuffix'];
					...
					// Get the mimetype, need to go to ../ parent folder cause... well we have to.
					$mimeType = mapMimeTypeFromUrl($file->getAbsolutePath(),
							"../" . $config['stream.mimefile']);
		...
		case "viewServerInfo":
			if (!checkBool($config['general.debug']))
				die(
						"You have to enable debugging in config by setting general.debug to true.");
		...
		case "downloadServerInfo":
			...
			$functions = array("ImagecreateFromJpeg", "ImageJpeg",
					"ImagecreateFromGif", "ImageGif", "ImagecreateFromPng",
					"ImagePng", "gzdeflate", "gzinflate");

			foreach ($functions as $function)
				echo $function . "="
						. (function_exists($function) ? "ok" : "missing")
						. "\r\n";
			...
			foreach ($man->getRootPaths() as $rootpath) {
				...
				echo $rootpath . "\r\n";
				echo "  is_readable="
						. (is_readable($rootpath) ? "readable" : "not readable")
						. "\r\n";
				echo "  is_writable="
						. (is_writable($rootpath) ? "writable" : "not writable")
						. "\r\n";
		...
		if ($cmd == "upload") {
			...
			if ($man->verifyPath($path)) {
				...
				if (isset($input["chunk"])) {
					...
					if (in_array("upload",
							explode(',', $config['general.disabled_tools']))
							|| !$file->canWrite()
							|| !checkBool($config["filesystem.writable"])) {
						$result
								->add("ACCESS_ERROR",
										$man
												->encryptPath(
								...
								->add("ACCESS_ERROR",
										$man
												->encryptPath(
														$file
																->getAbsolutePath()),
										$man->getInvalidFileMsg());
						return $result;
					}

					// Hack attempt
					if ($filename
							== $config['filesystem.local.access_file_name']) {
						$result
								->add("MCACCESS_ERROR",
										$man
												->encryptPath(
					...
					for ($i = 0; isset($_FILES['file' . $i]['tmp_name']); $i++) {
						...
						$ext = "";
						if (strpos(basename($_FILES['file' . $i]['name']), ".")
								> 0) {
							$ar = explode('.',
									basename($_FILES['file' . $i]['name']));
							$ext = array_pop($ar);
						}

						$file = &$man
								->getFile($path, $filename . "." . $ext, "",
										MC_IS_FILE);
						if ($man->verifyFile($file, "upload") < 0) {
							$result
									->add("ACCESS_ERROR",
				...
				$result
						->add("PATH_ERROR", $man->encryptPath($path),
								"{#error.upload_failed}");
	...
	function _insertFiles(&$man, $input) {
		$result = new Moxiecode_ResultSet(
				"name,path,url,size,type,created,modified,attribs,custom");
		$indata = array();
		...
		for ($i = 0; isset($input['path' . $i]); $i++) {
			...
			$man
					->dispatchEvent("onCustomInfo",
							array(&$file, "insert", &$custom));

			$status = $man->dispatchEvent("onInsertFile", array(&$file));
			$config = $file->getConfig();

			$attribs = ($file->canRead()
					&& checkBool($config["filesystem.readable"]) ? "R" : "-")
					. ($file->canWrite()
							&& checkBool($config["filesystem.writable"]) ? "W"
							: "-");
			$url = $man->removeTrailingSlash($config['preview.urlprefix'])
					. $man
							->convertPathToURI($file->getAbsolutePath(),
									$config["preview.wwwroot"]);

			$result
					->add($file->getName(),
							$man->encryptPath($file->getAbsolutePath()),
							utf8_encode($url), $file->getLength(), $ext,
							$file->getCreationDate(), $file->getLastModified(),
							$attribs, $custom);
		}
	...
	function _listDefault(&$man, $file, $input, &$result, $filter_root_path) {
		...
		if (isset($input['include_directory_pattern'])
				&& $input['include_directory_pattern'])
			$inputFileFilter
					->setIncludeDirectoryPattern(
							$input['include_directory_pattern']);

		if (isset($input['exclude_directory_pattern'])
				&& $input['exclude_directory_pattern'])
			$inputFileFilter
					->setExcludeDirectoryPattern(
							$input['exclude_directory_pattern']);

		if (isset($input['include_file_pattern'])
				&& $input['include_file_pattern'])
			$inputFileFilter
					->setIncludeFilePattern($input['include_file_pattern']);

		if (isset($input['exclude_file_pattern'])
				&& $input['exclude_file_pattern'])
			$inputFileFilter
					->setExcludeFilePattern($input['exclude_file_pattern']);
		...
		if (!$file->exists()) {
			...
			$result
					->setHeader("path",
							$man->encryptPath($file->getAbsolutePath()));
			$result
					->setHeader("visual_path",
							checkBool($config['general.user_friendly_paths']) ? $man
											->toVisualPath(
													$file->getAbsolutePath())
									: $man
											->encryptPath(
		...
		if ($file->isDirectory()) {
			...
			$fileFilter
					->setIncludeDirectoryPattern(
							$config['filesystem.include_directory_pattern']);
			$fileFilter
					->setExcludeDirectoryPattern(
							$config['filesystem.exclude_directory_pattern']);
			$fileFilter
					->setIncludeFilePattern(
							$config['filesystem.include_file_pattern']);
			$fileFilter
					->setExcludeFilePattern(
							$config['filesystem.exclude_file_pattern']);
			...
			if ($fileFilter->accept($file) <= 0) {
				...
				$result
						->setHeader("visual_path",
								checkBool(
										$config['general.user_friendly_paths']) ? $man
												->toVisualPath(
														$file
																->getAbsolutePath())
										: $man
			...
			else if (!checkBool($config["filesystem.list_directories"], true)
					|| (isset($input["only_files"])
			...
			$showparent = isset($input["no_parent"]) ? checkBool(
							$input["no_parent"]) : true;
			$showparent = $showparent && $man->verifyPath($file->getParent());

			if (!isset($input["only_dirs"]))
				$showparent = $showparent
						&& checkBool($config["filesystem.list_directories"],
								true);
			...
			if ($showparent && !isset($input["only_files"])) {
				...
				if ($filter_root_path
						&& getClassName($file) == 'moxiecode_localfileimpl') {
					if (!$man
							->isChildPath($filter_root_path, $file->getParent()))
						return $files;
				}

				if ($file->getAbsolutePath() != $filter_root_path)
					$result
							->add("..", $man->encryptPath($file->getParent()),
									-1, "parent", "", "", "", array());
			}
		} else
			trigger_error(
	...
	function _listFiles(&$man, $input) {
		$result = new Moxiecode_ResultSet(
				"name,path,size,type,created,modified,attribs,custom");
		$config = $man->getConfig();
		$files = array();
		$rootNames = $man->_getRootNames();
		$filterRootPath = isset($input["root_path"])
				&& $input["root_path"] != null ? $man
						->toAbsPath($man->decryptPath($input["root_path"]))
				: null;
		...
		if (isset($input["path"]) && $input["path"]) {
			...
			if (isset($input['remember_last_path'])) {
				...
				if ($remember) {
					if (isset(
							$_COOKIE["MCManager_" . $man->getType()
									. "_lastPath"])
							&& $input["path"] == '{default}') {
						$tmpPath = $_COOKIE["MCManager_" . $man->getType()
								. "_lastPath"];
						...
						if ($man->getFSFromPath($input["path"]) == "file")
							setcookie(
									"MCManager_" . $man->getType()
											. "_lastPath", $input["path"],
									time() + (3600 * 24 * 30));
						// 30 days
					}
				}
			...
			$result
					->setHeader("visual_path",
							checkBool($config['general.user_friendly_paths']) ? $man
											->toVisualPath($input["path"])
									: $man->encryptPath($input["path"]));

			// Move path inside rootpath if it's localfs
			if ($filterRootPath
					&& $man->getFSFromPath($input["path"]) == 'file') {
			...
			$files = $this
					->_listDefault($man, $file, $input, $result,
							$filterRootPath);
		} else {
		...
		if (isset($input["page_size"])) {
			if ($file->getAbsolutePath() != $filterRootPath
					&& $man->verifyPath($file->getParent()))
			...
			$files = array_slice($files, ($input["page"] * $pageSize),
					$pageSize);
		}
		...
		foreach ($files as $file) {
			...
			if ($filterRootPath && $listFS == 'file') {
				if (!$man
						->isChildPath($filterRootPath, $file->getAbsolutePath()))
					continue;
			}
			...
			$attribs = ($file->canRead()
					...
					. ($file->canWrite()
							&& checkBool($config["filesystem.writable"]) ? "W"
							: "-");
			$cdate = date($config['filesystem.datefmt'],
					$file->getCreationDate());
			$mdate = date($config['filesystem.datefmt'],
			...
			$result
					->add(utf8_encode($name), $filePath,
							$file->isFile() ? $file->getLength() : -1, $type,
							$cdate, $mdate, $attribs, $custom);
		}
	...
	function _createDirs(&$man, $input) {
		...
		if (checkBool($config["general.demo"])) {
			$result
					->add("DEMO_ERROR",
							$man->encryptPath($dir->getAbsolutePath()),
							"{#error.demo}");
			return $result->toArray();
		}
	...
	function _getConfig(&$man, $input) {
		...
		if (!isset($input['path']) && isset($input['debug'])
				&& checkBool($globalConfig['general.debug']))

...
class Moxiecode_LocalFileImpl extends Moxiecode_BaseFile {
	 ...
	 */
	function Moxiecode_LocalFileImpl(&$manager, $absolute_path,
			$child_name = "", $type = MC_IS_FILE) {
		$this->_manager = &$manager;
		$this->_type = $type;

		if ($child_name != "")
			$this->_absPath = $this->_manager
					->removeTrailingSlash($absolute_path) . "/" . $child_name;
		else
		...
		if (strpos($this->_absPath, '..') !== false)
			$this->_absPath = $this->_manager
					->toUnixPath(realpath($this->_absPath));
	}
	...
	function importFile($local_absolute_path = "") {
		...
		if ($this->_events) {
			if ($this->isFile()) {
				$this->_manager
						->dispatchEvent("onBeforeFileAction",
								array(ADD_ACTION, &$this));
				$this->_manager
						->dispatchEvent("onFileAction",
								array(ADD_ACTION, &$this));
			} else {
				$this->_manager
						->dispatchEvent("onBeforeFileAction",
								array(MKDIR_ACTION, &$this));
				$this->_manager
						->dispatchEvent("onFileAction",
								array(MKDIR_ACTION, &$this));
			}
		...
		// Chmod the file
		if (($mask = $this
				->_getConfigValue("filesystem.local.file_mask", false))
				!== false)
			@chmod($this->_manager->toOSPath($this->_absPath), intval($mask, 8));

		// Chown the file
		if (($owner = $this
				->_getConfigValue("filesystem.local.file_owner", false))
				!== false)
	...
	function exists() {
		...
		return ($this->_statCache['exists'] = file_exists(
				$this->_manager->toOSPath($this->_absPath)));
	}
	...
	function isDirectory() {
		...
		return ($this->_statCache['is_dir'] = is_dir(
				$this->_manager->toOSPath($this->_absPath)));
	}
	...
	function isFile() {
		...
		return ($this->_statCache['is_file'] = is_file(
				$this->_manager->toOSPath($this->_absPath)));
	}
	...
	function canRead() {
		...
		return ($this->_statCache['is_readable'] = is_readable(
				$this->_manager->toOSPath($this->_absPath)));
	}
	...
	function canWrite() {
		...
		// Other OS:es
		return ($this->_statCache['is_writeable'] = is_writeable(
				$this->_manager->toOSPath($this->_absPath)));
	}
	...
	function copyTo(&$dest) {
		...
		if ($this->isDirectory()) {
			...
			$treeHandler = new _LocalCopyDirTreeHandler($this->_manager, $this,
					$dest, $handle_as_add_event);
			...
			if ($this->_events)
				$this->_manager
						->dispatchEvent("onBeforeFileAction",
								array(COPY_ACTION, &$this, &$dest));

			$status = $this->_absPath == $dest->_absPath
					|| copy($this->_manager->toOSPath($this->_absPath),
							$this->_manager->toOSPath($dest->_absPath));

			// Chmod the file
			if (($mask = $dest
					->_getConfigValue("filesystem.local.file_mask", false))
					!== false)
				@chmod($this->_manager->toOSPath($dest->_absPath),
						intval($mask, 8));
			...
			if (($owner = $dest
					...
					!== false)
				$this
						->_chown($this->_manager->toOSPath($dest->_absPath),
								$owner);
	...
	function delete($deep = false) {
		...
		if (is_dir($this->_manager->toOSPath($this->_absPath))) {
			if ($this->_events)
				$this->_manager
						->dispatchEvent("onBeforeFileAction",
								array(RMDIR_ACTION, &$this));

			if ($deep) {
				// Get all the files
				$treeHandler = new _LocalDeleteDirTreeHandler($this->_manager,
						$this);
				$this->listTree($treeHandler);
			...
			if (($status = rmdir($this->_manager->toOSPath($this->_absPath)))
					&& ($this->_events))
				$this->_manager
						->dispatchEvent("onFileAction",
								array(RMDIR_ACTION, &$this));
			...
			if ($this->_events)
				$this->_manager
						->dispatchEvent("onBeforeFileAction",
								array(DELETE_ACTION, &$this));

			if (($status = unlink($this->_manager->toOSPath($this->_absPath)))
					&& ($this->_events))
				$this->_manager
						->dispatchEvent("onFileAction",
								array(DELETE_ACTION, &$this));
	...
	function &listFilesFiltered(&$filter) {
		...
		if ($fHnd = opendir($this->_manager->toOSPath($dir))) {
			...
			foreach ($dirArray as $adir) {
				$file = new Moxiecode_LocalFileImpl($this->_manager,
						$dir . "/" . $adir);
	...
	function listTree(&$file_tree_handler) {
		$this
				->_listTree($this, $file_tree_handler,
						new Moxiecode_DummyFileFilter(), 0);
	}
	...
	function mkdir() {
		...
		$tpl = $this
				->_getConfigValue("filesystem.local.directory_template", false);

		if ($tpl)
			$status = copy($this->_manager->toAbsPath($tpl),
					$this->_manager->toOSPath($this->_absPath));
		else
		...
		// Chmod the dir
		if (($mask = $this
				->_getConfigValue("filesystem.local.directory_mask", false))
				!== false)
			@chmod($this->_manager->toOSPath($this->_absPath), intval($mask, 8));

		// Chown the dir
		if (($owner = $this
				->_getConfigValue("filesystem.local.directory_owner", false))
				!== false)
			$this->_chown($this->_manager->toOSPath($this->_absPath), $owner);

		if ($status && $this->_events)
			$this->_manager
					->dispatchEvent("onFileAction", array(MKDIR_ACTION, &$this));
	...
	function renameTo(&$dest) {
		...
		if ($this->_events)
			$this->_manager
					->dispatchEvent("onBeforeFileAction",
							array(RENAME_ACTION, &$this, &$dest));

		if ($this->_absPath != $dest->_absPath)
			$status = rename($this->_manager->toOSPath($this->_absPath),
					$this->_manager->toOSPath($dest->_absPath));
		...
		if (($mask = $dest
				->_getConfigValue(
						"filesystem.local." . ($isFile ? "file" : "directory")
								. "_mask", false)) !== false)
	...
	function _listTree($file, &$file_tree_handler, &$file_filter, $level) {
		...
		if ($file_filter->accept($file)) {
			...
			if ($state == $file_tree_handler->ABORT
					|| $state == $file_tree_handler->ABORT_FOLDER)
				return $state;
		}
		...
		foreach ($files as $file) {
			if ($file_filter->accept($file)) {
				if ($file->isFile()) {
					...
					$state = $this
							->_listTree($file, $file_tree_handler,
									$file_filter, ++$level);
					--$level;
				}
			}
	...
	function getConfig() {
		...
		if (!$this->_config) {
			...
			while ($file->exists() && $file->getAbsolutePath() != "/") {
				$accessFile = new Moxiecode_LocalFileImpl($this->_manager,
						$file->getAbsolutePath(),
						$globalConf["filesystem.local.access_file_name"]);
			...
			foreach (array_reverse($accessFiles) as $accessFile) {
				...
				if (($fp = fopen(
						$this->_manager
								->toOSPath($accessFile->getAbsolutePath()),
						"r"))) {
					while (!feof($fp)) {
						$line = trim(fgets($fp));
						...
						if (($pos = strpos($line, "=")) !== FALSE)
							$config[substr($line, 0, $pos)] = substr($line,
									$pos + 1);
					}
				...
				$curDir = $this->isFile() ? $this->getParent()
				...
				foreach ($config as $key => $value) {
					...
					if ((in_array($key, $allowoverrideKeys) || $validAllKey)
							&& !in_array($key, $denyOverrideKeys)) {
						if (strpos($value, '${') !== false)
							$value = str_replace('${configpath}',
									$accessFile->getParent(), $value);
			...
			foreach ($this->_config as $key => $value) {
				if (!is_array($value) && strpos($value, '${') !== false) {
					...
					$value = str_replace('${rootpath}',
							$this->_manager
									->toUnixPath($this->_manager->_rootPaths[0]),
							$value);

					// Handle multiple rootpaths
					for ($i = 0; $i < count($this->_manager->_rootPaths); $i++)
						$value = str_replace('${rootpath' . $i . '}',
								$this->_manager
										->toUnixPath(
												$this->_manager->_rootPaths[$i]),
								$value);
...
class _LocalCopyDirTreeHandler extends Moxiecode_FileTreeHandler {
	...
	function _LocalCopyDirTreeHandler(&$manager, $from_file, $dest_file,
			$handle_as_add_event) {
		$this->_manager = $manager;
	...
	function handle($file, $level) {
		...
		$toPath .= substr($file->getAbsolutePath(),
				strlen($this->_fromFile->getAbsolutePath()));

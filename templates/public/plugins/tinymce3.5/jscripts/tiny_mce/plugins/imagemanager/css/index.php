...
$compressor = new Moxiecode_CSSCompressor(
		array('expires_offset' => 3600 * 24 * 10, 'disk_cache' => true,
				'cache_dir' => '_cache', 'gzip_compress' => true,
				'remove_whitespace' => false, 'charset' => 'UTF-8',
				'name' => $theme . "_" . $package, 'convert_urls' => true));
...
if ($resources->isDebugEnabled() || checkBool($config["general.debug"])) {
	...
	foreach ($files as $file)
		$compressor
				->addFile($file->getPath(), $file->isRemoveWhiteSpaceEnabled());

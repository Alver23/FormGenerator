...
class Moxiecode_HistoryPlugin extends Moxiecode_ManagerPlugin {
	...
	function onInsertFile(&$man, $file) {
		...
		$type = $man->getType();
		$maxhistory = isset($config["history.max"]) ? $config["history.max"]
				: $this->_maxhistory;
	...
	function setCookieData($type, $val) {
		setcookie("MCManagerHistoryCookie_" . $type, $val, time()
				+ (3600 * 24 * 30)); // 30 days
	}

	function onClearHistory(&$man) {
		setcookie("MCManagerHistoryCookie_" . $man->getType(), "", time()
				- 3600); // 1 hour ago
		return true;
	}
}
...
class Moxiecode_HistoryFile extends Moxiecode_BaseFileImpl {
	function Moxiecode_HistoryFile(&$manager, $absolute_path, $child_name = "",
			$type = MC_IS_FILE) {

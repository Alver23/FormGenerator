...
class Moxiecode_BasicFileFilter extends Moxiecode_FileFilter {
	...
	function accept(&$file) {
		...
		if ($isFile) {
			...
			if ($this->_excludeFilePattern
					&& preg_match($this->_excludeFilePattern, $name)) {
				if ($this->_debug)
					debug(
							"File \"" . $absPath
									. "\" denied by \"excludeFilePattern\".");

				return BASIC_FILEFILTER_INVALID_NAME;
			}

			// Handle include pattern
			if ($this->_includeFilePattern
					&& !preg_match($this->_includeFilePattern, $name)) {
				if ($this->_debug)
					debug(
							"File \"" . $absPath
									. "\" denied by \"includeFilePattern\".");
			...
			// Handle exclude pattern
			if ($this->_excludeDirectoryPattern
					&& preg_match($this->_excludeDirectoryPattern, $name)) {
				if ($this->_debug)
					debug(
							"File \"" . $absPath
									. "\" denied by \"excludeDirectoryPattern\".");

				return BASIC_FILEFILTER_INVALID_NAME;
			}

			// Handle include pattern
			if ($this->_includeDirectoryPattern
					&& !preg_match($this->_includeDirectoryPattern, $name)) {
				if ($this->_debug)
					debug(
							"File \"" . $absPath
									. "\" denied by \"includeDirectoryPattern\".");
		...
		if ($this->_includeWildcardPattern
				&& !$this->_fnmatch($this->_includeWildcardPattern, $name)) {
			if ($this->_debug)
				debug(
						"File \"" . $absPath
								. "\" denied by \"includeWildcardPattern\".");

			return BASIC_FILEFILTER_INVALID_NAME;
		}

		// Handle exclude wildcard pattern
		if ($this->_excludeWildcardPattern
				&& $this->_fnmatch($this->_excludeWildcardPattern, $name)) {
			if ($this->_debug)
				debug(
						"File \"" . $absPath
								. "\" denied by \"excludeWildcardPattern\".");
	...
	function _fnmatch($pattern, $file) {
		return ereg($this->_fnmatch2regexp(strtolower($pattern)),
				strtolower($file));
	}

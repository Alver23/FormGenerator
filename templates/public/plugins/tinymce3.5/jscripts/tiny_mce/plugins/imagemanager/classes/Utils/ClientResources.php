...
class Moxiecode_ClientResources {
	...
	function getFiles($package) {
		return isset($this->_files[$package]) ? $this->_files[$package]
				: array();
	}
	...
	function _saxStartElement($parser, $name, $attrs) {
		...
		case "FILE":
			$this->_files[$this->_packageID][] = new Moxiecode_ClientResourceFile(
					...
					!isset($attrs["KEEPWHITESPACE"])
							|| $attrs["KEEPWHITESPACE"] != "yes",
...
class Moxiecode_ClientResourceFile {
	...
	function Moxiecode_ClientResourceFile($id, $path, $remove_whitespace,
			$content_type) {
		$this->_id = $id;
		$this->_path = $path;

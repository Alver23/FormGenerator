...
if ($mamboUser->id == 0) {
	...
	$mamboUser->username = strval(
			mosGetParam($_SESSION, 'session_username', ''));
	$mamboUser->usertype = strval(
			mosGetParam($_SESSION, 'session_usertype', ''));
}
...
class Moxiecode_JoomlaAuthenticator extends Moxiecode_ManagerPlugin {
	...
	function onAuthenticate(&$man) {
		...
		return preg_match($config['JoomlaAuthenticator.valid_users'],
				$mamboUser->username);
	}
...
$man
		->registerPlugin("JoomlaAuthenticator",
				new Moxiecode_JoomlaAuthenticator());

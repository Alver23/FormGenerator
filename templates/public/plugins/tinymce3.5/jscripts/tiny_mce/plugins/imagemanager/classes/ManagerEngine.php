...
class Moxiecode_ManagerEngine {
	...
	function setConfig($config, $setup_values = true) {
		...
		if ($setup_values) {
			...
			if ($config['filesystem.path'] == "")
				...
				$config['filesystem.path'] = $this
						->removeTrailingSlash(
								$this->toAbsPath($config['filesystem.path']));

			// Setup absolute wwwroot
			if (isset($config['preview.wwwroot']) && $config['preview.wwwroot'])
				$config['preview.wwwroot'] = $this
						->toUnixPath(
								$this->toAbsPath($config['preview.wwwroot']));
			else
			...
			if ($config["preview.urlprefix"]) {
				if (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == "on")
					$config["preview.urlprefix"] = str_replace("{proto}",
							"https", $config["preview.urlprefix"]);
				else
					$config["preview.urlprefix"] = str_replace("{proto}",
							"http", $config["preview.urlprefix"]);

				$config["preview.urlprefix"] = str_replace("{host}",
						$_SERVER['HTTP_HOST'], $config["preview.urlprefix"]);
				$config["preview.urlprefix"] = str_replace("{port}",
						$_SERVER['SERVER_PORT'], $config["preview.urlprefix"]);
			}
		}
	...
	function getPluginPaths() {
		...
		if (strpos($config["authenticator"], "+")
				|| strpos($config["authenticator"], "|"))
			$authenticators = preg_split("/\+|\|/i", $config["authenticator"],
		...
		if (count($authenticators) != 0) {
			...
			if (!isset($this->_plugins[strtolower($config["authenticator"])])) {
				if (file_exists(
						$basepath . "Authenticators/"
								. $config["authenticator"] . ".php"))
					$out[] = "classes/Authenticators/"
							. $config["authenticator"] . ".php";
				else if (file_exists(
						$basepath . "../plugins/" . $config["authenticator"]
								. ".php"))
					$out[] = "plugins/" . $config["authenticator"] . ".php";
				else
					$out[] = "plugins/" . $config["authenticator"] . "/"
							. $config["authenticator"] . ".php";
			}
		}
	...
	function &getJSConfig($config = false, $prefixes = '*') {
		$encrypted = array("filesystem.path", "filesystem.rootpath",
				...
				if (in_array("*", $prefixes) || in_array($prefix, $prefixes)) {
					foreach ($names as $key) {
						...
						if (in_array($key, $encrypted))
							$jsConfig[$key] = ""
									. $this->encryptPath($config[$key]);
						else
							$jsConfig[$key] = "" . is_bool($config[$key]) ? ($config[$key] ? "true"
											: "false") : $config[$key];
					}
				}
			}
	...
	function decryptPath($path) {
		...
		$path = str_replace("{default}", $this->_config["filesystem.path"],
				$path);
	...
	function isAuthenticated() {
		...
		if ($authenticators != "" && $authenticators != "BaseAuthenticator") {
			if (strpos($authenticators, "|") && strpos($authenticators, "+"))
				trigger_error(
						"You can not use both + and | at the same time for adding authenticators.",
						FATAL);
			...
			if (strpos($authenticators, "+")) {
				$authArray = preg_split("/\+/i", $authenticators, -1,
						PREG_SPLIT_NO_EMPTY);
				...
				foreach ($authArray as $auth) {
					if (!$this->hasPlugin($auth))
						trigger_error(
								"Authenticator \"" . htmlentities($auth)
										. "\" was not found.", FATAL);
				}
	...
	function dispatchEvent($event, $args = false) {
		...
		for ($i = 0; $i < count($keys); $i++) {
			...
			case "onBeforeFileAction":
				...
				if (!$plugin
						->onBeforeFileAction($this, $args[0], $args[1],
								$args[2]))
					return false;
	...
	function getSiteRoot() {
		// Check config
		if (isset($this->_config['preview.wwwroot'])
				&& $this->_config['preview.wwwroot'])
			return $this
					->toUnixPath(realpath($this->_config['preview.wwwroot']));

		// Try script file
		if (isset($_SERVER["SCRIPT_NAME"])
				&& isset($_SERVER["SCRIPT_FILENAME"])) {
			$path = str_replace($this->toUnixPath($_SERVER["SCRIPT_NAME"]), "",
					$this->toUnixPath($_SERVER["SCRIPT_FILENAME"]));
		...
		if (isset($_SERVER["SCRIPT_NAME"])
			...
			$path = str_replace($this->toUnixPath($_SERVER["SCRIPT_NAME"]), "",
					str_replace("//", "/",
							$this->toUnixPath($_SERVER["PATH_TRANSLATED"])));
		...
		trigger_error(
				"Could not resolve WWWROOT path, please set an absolute path in preview.wwwroot config option. Check the Wiki documentation for details.",
				FATAL);
	...
	function convertPathToURI($abs_path, $root = false) {
		...
		if (!$root) {
			trigger_error(
					"Could not resolve WWWROOT path, please set an absolute path in preview.wwwroot config option.",
					FATAL);
			die();
		}
		...
		if ($root == "/") {
			if ($log && $log->isDebugEnabled())
				$log
						->info(
								"ConvertPathToURI: SiteRoot=" . $root
										. ", Path: " . $abs_path . " -> URI: "
										. $abs_path);
		...
		if ($log && $log->isDebugEnabled())
			$log
					->info(
							"ConvertPathToURI: SiteRoot=" . $root . ", Path: "
									. $abs_path . " -> URI: " . $uri);
	...
	function convertURIToPath($uri, $root = false) {
		...
		if ($log && $log->isDebugEnabled())
			$log
					->info(
							"ConvertURIToPath: SiteRoot=" . $root . ", URI: "
									. $uri . " -> Path: "
									. $this->removeTrailingSlash($root) . $uri);
	...
	function verifyPath($path) {
		...
		if (!isset($this->_fileSystems[$fs])) {
			trigger_error(
					$this
							->getLangItem("error", "no_filesystem",
									array("path" => $path)), FATAL);
			die();
		}
	...
	function verifyFile($file, $action = false, $config = false) {
		...
		if ($status != BASIC_FILEFILTER_ACCEPTED) {
			if ($status == BASIC_FILEFILTER_INVALID_NAME) {
				if ($file->isFile()
						&& isset($config['filesystem.invalid_file_name_msg']))
					$this->_invalidFileMsg = $config['filesystem.invalid_file_name_msg'];
				else if (!$file->isFile()
						&& isset(
								$config['filesystem.invalid_directory_name_msg']))
		...
		if ($action) {
			if (isset($config[$action . '.include_directory_pattern']))
				$fileFilter
						->setIncludeDirectoryPattern(
								$config[$action . '.include_directory_pattern']);

			if (isset($config[$action . '.exclude_directory_pattern']))
				$fileFilter
						->setExcludeDirectoryPattern(
								$config[$action . '.exclude_directory_pattern']);

			if (isset($config[$action . '.include_file_pattern']))
				$fileFilter
						->setIncludeFilePattern(
								$config[$action . '.include_file_pattern']);

			if (isset($config[$action . '.exclude_file_pattern']))
				$fileFilter
						->setExcludeFilePattern(
								$config[$action . '.exclude_file_pattern']);

			if (isset($config[$action . '.extensions']))
				$fileFilter
						->setIncludeExtensions($config[$action . '.extensions']);
		} else
	...
	function toAbsPath($path, $basepath = false) {
		...
		$path = $this
				->toUnixPath(
						$this
								->addTrailingSlash(
										$this->toUnixPath(realpath($basepath)))
								. $path);
	...
	function &getLogger() {
		if (!$this->_logger) {
			...
			$log
					->setFileName(
							$this->getConfigItem("log.filename", "{level}.log"));
			$log
					->setFormat(
							$this
									->getConfigItem("log.format",
											"[{time}] [{level}] {message}"));

...
class Moxiecode_LanguagePack {
	...
	function loadXML($data) {
		...
		xml_set_element_handler($this->_parser, "_saxStartElement",
				"_saxEndElement");
		xml_set_character_data_handler($this->_parser, "_saxCharacterData");
		xml_parser_set_option($this->_parser, XML_OPTION_TARGET_ENCODING,
				"UTF-8");

		if (!xml_parse($this->_parser, $data, true))
			trigger_error(
					sprintf(
							"Language pack loading failed, XML error: %s at line %d.",
	...
	function save($file, $enc = "UTF-8") {
		if (($fp = @fopen($file, "w"))) {
			...
			trigger_error("Could not open XML for writing: " . $file,
					E_USER_ERROR);
	}
	...
	function get($target, $name) {
		return isset(
				$this->_items[$this->_currentLanguage]["data"][$target][$name]) ? $this
						->_items[$this->_currentLanguage]["data"][$target][$name]
				: ("$" . $name . "$");
	}
	...
	function createLanguage($lang, $dir, $title) {
		...
		$this->_items[$lang] = array("dir" => $dir, "title" => $title,
				"data" => array());
	}
	...
	function _saxStartElement($parser, $name, $attrs) {
		...
		case "LANGUAGE":
			...
			$this->_items[$this->_currentLanguage] = array(
					"dir" => $this->_currentDir,
					"title" => $this->_tagAttrs["TITLE"], "data" => array());
			break;

		case "GROUP":
			$this->_currentTarget = $this->_tagAttrs["TARGET"];
			$this
					->_items[$this->_currentLanguage]["data"][$this
							->_tagAttrs["TARGET"]] = array();
			break;
		}
	}
	...
	function _saxEndElement($parser, $name) {
		...
		case "ITEM":
			if (count($matches) != 0) {
				...
				if (!isset(
						$this
								->_cdataArr[$this->_currentLanguage][$this
										->_currentTarget]))
					$this
							->_cdataArr[$this->_currentLanguage][$this
									->_currentTarget] = array();

				$this
						->_cdataArr[$this->_currentLanguage][$this
								->_currentTarget][$this->_tagAttrs["NAME"]] = true;
			}

			$this
					->_items[$this->_currentLanguage]["data"][$this
							->_currentTarget][$this->_tagAttrs["NAME"]] = $this
					->_tagContent;
			break;
		case "AUTHOR":
		...
		case "VERSION":
			...
			$this->_header["releasedate"] = trim(
					$this->_tagAttrs["RELEASEDATE"]);
			break;
		case "DESCRIPTION":
	...
	function xmlEncode($str) {
		if (strtolower($this->_encoding) == "utf-8")
			return utf8_encode(
					htmlspecialchars($str, ENT_QUOTES, $this->_encoding));
	...
	function toString($enc = "") {
		...
		$doc .= '<language-pack>' . "\n";
		$doc .= '	<header>' . "\n";
		$doc .= '		<author>' . $this->xmlEncode($this->_header["author"])
				. '</author>' . "\n";
		$doc .= '		<version major="'
				. $this->xmlEncode($this->_header["major"]) . '" minor="'
				. $this->xmlEncode($this->_header["minor"]) . '" releasedate="'
				. $this->xmlEncode($this->_header["releasedate"]) . '" />'
				. "\n";
		$doc .= '		<description>'
				. $this->xmlEncode($this->_header["description"])
				. '</description>' . "\n";
		$doc .= '	</header>' . "\n";

		foreach ($this->_items as $code => $language) {
			$doc .= '	<language code="' . $this->xmlEncode($code)
					. '" dir="' . $this->xmlEncode($language["dir"])
					. '" title="' . $this->xmlEncode($language["title"]) . '">'
					. "\n";
			foreach ($language["data"] as $target => $group) {
				...
				foreach ($group as $name => $item) {
					if (isset($this->_cdataArr[$code])
							&& isset($this->_cdataArr[$code][$target])
							&& isset($this->_cdataArr[$code][$target][$name])
							&& $this->_cdataArr[$code][$target][$name]) {
						$doc .= '			<item name="'
								. $this->xmlEncode($name) . '"><![CDATA['
								. $item . ']]></item>' . "\n";
					} else
						$doc .= '			<item name="'
								. $this->xmlEncode($name) . '">'
								. $this->xmlEncode($item) . '</item>' . "\n";
				}
				$doc .= '		</group>' . "\n";
			}
			$doc .= '	</language>' . "\n";

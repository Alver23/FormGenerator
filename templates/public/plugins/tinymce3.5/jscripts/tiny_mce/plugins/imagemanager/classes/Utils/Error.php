...
function JSONErrorHandler($errno, $errstr, $errfile, $errline, $errcontext) {
	...
	$data = $MCErrorHandler
			->handleError($errno, $errstr, $errfile, $errline, $errcontext);
...
function StreamErrorHandler($errno, $errstr, $errfile, $errline, $errcontext) {
	...
	if ($data['break']) {
		if ($_SERVER["REQUEST_METHOD"] == "GET") {
			...
			echo '<html><body><script type="text/javascript">parent.handleJSON('
					. $json->encode($result) . ');</script></body></html>';
			die();
		}
	}
}

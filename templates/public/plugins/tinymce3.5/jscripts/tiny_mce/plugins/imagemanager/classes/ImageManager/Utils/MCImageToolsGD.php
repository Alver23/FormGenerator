...
class ImageToolsGD {
	 ...
	 * @return Bool true or false depending on success or not.
	 */
	function rotateImage($source, $target, $ext, $angle = "90", $bgc = "0",
			$quality = "") {
		if (!$this->canEdit($ext))
			return false;
	 ...
	 * @return Bool true or false depending on success or not.
	 */
	function cropImage($source, $target, $top, $left, $width, $height, $ext,
			$quality = "") {
		if (!$this->canEdit($ext))
			return false;
	 ...
	 * @return Bool true or false depending on success or not.
	 */
	function flipImage($source, $target, $ext, $vert = false, $hori = false,
			$quality = "") {
		if (!$this->canEdit($ext))
			return false;
	...
	function _cropGif() {
		...
		imagefilledrectangle($image, 0, 0, $this->_width, $this->_width,
				$transparent);

		imagecolortransparent($image, $transparent);

		ImageCopyResampled($image, $source, 0, 0, $this->_left, $this->_top,
	...
	function _resizeGif() {
		...
		ImageCopyResampled($image, $source, 0, 0, 0, 0, $this->_width,
				$this->_height, ImageSX($source), ImageSY($source));
		ImageDestroy($source);
	...
	function _flipPng() {
		...
		if ($this->_hori) {
			for ($i = 0; $i < $width; $i++)
				ImageCopyResampled($image, $source, $width - $i - 1, 0, $i, 0,
						1, $height, 1, $height);

			if ($this->_vert)
				ImageCopyResampled($source, $image, 0, 0, 0, 0, $width,
						$height, $width, $height);
		}

		if ($this->_vert) {
			for ($i = 0; $i < $height; $i++)
				ImageCopyResampled($image, $source, 0, $height - $i - 1, 0, $i,
						$width, 1, $width, 1);
		}
			...
			// Read chunks
				while (!feof($fp)) {
					...
					if ($chunk['type'] == 'IHDR') {
						$buff = fread($fp, 13);
						$header = unpack(
								'Nwidth/Nheight/Cbits/Ctype/Ccompression/Cfilter/Cinterlace',
								$buff);
						fclose($fp);
						return $header;
					}
	...
	function getEditSupport($type) {
		...
		case "jpg":
			$resize = $this
					->hasFunctions(
							"ImageCreateTrueColor,ImageCopyResampled,ImageSX,ImageSY");
			$crop = $this
					->hasFunctions("ImageCreateTrueColor,ImageCopyResampled");
			break;

		case "gif":
			$resize = $this
					->hasFunctions(
							"imagecolorallocate,imagefilledrectangle,imagecolortransparent,ImageCopyResampled,ImageSX,ImageSY");
			$crop = $this
					->hasFunctions(
							"imagecolorallocate,imagefilledrectangle,imagecolortransparent,ImageCopyResampled");
			break;
		...
		case "png":
			...
			$crop = $this
					->hasFunctions(
							"imagealphablending,imagesavealpha,ImageColorTransparent,ImageCopyResampled,ImageCreateTrueColor");
			break;
		}
		...
		foreach ($chunks as $chunk) {
			...
			foreach ($items as $item) {
				...
				default:
					...
					if (preg_match('/\s?([0-9]+)\s?x([0-9]+)\s?/', $item,
							$matches)) {
						$actions[] = "resize";
			...
			foreach ($actions as $action) {
				...
				//debug($path, $outPath);
					$this
							->resizeImage($path, $outPath, $newWidth,
									$newHeight, "", $quality);
					break;
				}
			}
	...
	function canEdit($type) {
		...
		foreach ($gdUsedFunctions as $function) {
			if (!function_exists($function)) {
				trigger_error("Function " . $function . " does not exist.",
						WARNING);
				return false;
			}
		}

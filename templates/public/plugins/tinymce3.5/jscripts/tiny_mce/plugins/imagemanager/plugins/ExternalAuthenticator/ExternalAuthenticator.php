...
class Moxiecode_ExternalAuthenticator extends Moxiecode_ManagerPlugin {
	...
	function onAuthenticate(&$man) {
		...
		$prefix = isset($config['ExternalAuthenticator.session_prefix']) ? $config['ExternalAuthenticator.session_prefix']
				: "mcmanager_";
		$useCookie = isset($config['ExternalAuthenticator.use_cookie']) ? $config['ExternalAuthenticator.use_cookie']
						== true : true;
		...
		if ($dir == "language") {
			...
			if (isset(
					$_SESSION[$prefix
							. "ExternalAuthenticator_general__language"]))
				$config["general.language"] = $_SESSION[$prefix
						. "ExternalAuthenticator_general__language"];
		...
		if ($dir == "rpc" || $dir == "stream") {
			if (isset($_SESSION[$prefix . 'ExternalAuthenticator'])
					&& $_SESSION[$prefix . 'ExternalAuthenticator'] == true) {
				if (!$useCookie
						|| isset($_COOKIE[$prefix . 'enabled'])
								&& $_COOKIE[$prefix . 'enabled']
										== md5(
												$secretKey
														. $_SERVER['REMOTE_ADDR'])) {
					foreach ($_SESSION as $key => $value) {
						if (strpos($key, $prefix . "ExternalAuthenticator_")
								=== 0) {
							$key = str_replace("__", ".", $key);
							$key = substr($key,
									strlen($prefix . "ExternalAuthenticator_"));
							$config[$key] = $value;
						}
					}
		...
		if (isset($_POST['key'])) {
			...
			if ($_POST['key'] == md5($data . $secretKey)) {
				...
				if ($useCookie)
					setcookie($prefix . 'enabled',
							md5($secretKey . $_SERVER['REMOTE_ADDR']), 0, '/');
		...
		if ($dir == "rpc" || $dir == "stream") {
			...
			// Make it absolute
			if (strpos($authURL, "/") === 0)
				$authURL = $prot . "://" . $_SERVER['HTTP_HOST'] . $port
						. $authURL;

			$returnURL = $prot . "://" . $_SERVER['HTTP_HOST'] . $port
					. dirname(dirname($_SERVER['PHP_SELF']))
					. "/index.php?type=" . $man->getType();
			$config['authenticator.login_page'] = $authURL . "?return_url="
					. urlencode($returnURL);
			return false;
		}
		...
		$returnURL = $prot . "://" . $_SERVER['HTTP_HOST'] . $port
				. $_SERVER['PHP_SELF'] . "?type=" . $man->getType();
...
$man
		->registerPlugin("ExternalAuthenticator",
				new Moxiecode_ExternalAuthenticator());

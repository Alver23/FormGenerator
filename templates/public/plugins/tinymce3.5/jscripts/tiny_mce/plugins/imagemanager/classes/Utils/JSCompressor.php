...
class Moxiecode_JSCompressor {
	...
	function Moxiecode_JSCompressor($settings = array()) {
		...
		$default = array('expires_offset' => '10d', 'disk_cache' => true,
				'cache_dir' => '_cache', 'gzip_compress' => true,
				'remove_whitespace' => true, 'charset' => 'UTF-8',
				'patch_ie' => true, 'remove_firebug' => false, 'name' => '');
	...
	function compress() {
		...
		if ($this->_settings['name'])
			$cacheFile .= preg_replace('/[^a-z0-9_]/i', '',
					$this->_settings['name']);
		else
			$cacheFile .= md5($key);
		...
		if ($this->_settings['gzip_compress']
						...
						. gmdate("D, d M Y H:i:s",
								time()
										+ $this
												->_parseTime(
														$this
																->_settings['expires_offset']))
						. " GMT");
		header(
				"Cache-Control: public, max-age="
						. $this->_parseTime($this->_settings['expires_offset']));

		// Output explorer workaround or compressed file
		if (!isset($_GET["gz"]) && $supportsGzip
				&& $this->_settings['patch_ie']
			...
			echo 'gz.open("GET", "' . $url
					. '", false);gz.send(null);eval(gz.responseText);';
			die();
		}

		// Use cached file
		if ($this->_settings['disk_cache'] && file_exists($cacheFile)
				&& @filemtime($cacheFile) == $this->_lastUpdate) {
			if ($supportsGzip)
	...
	function _removeWhiteSpace($content) {
		...
		$content = preg_replace_callback('/\\\\(\"|\'|\\/)/',
				array(&$this, '_encode'), $content); // Replace all \/, \", \' with tokens
		$content = preg_replace_callback(
				'/(\'[^\'\\n\\r]*\')|("[^"\\n\\r]*")|(\\s+(\\/[^\\/\\n\\r\\*][^\\/\\n\\r]*\\/g?i?))|([^\\w\\x24\\/\'"*)\\?:]\\/[^\\/\\n\\r\\*][^\\/\\n\\r]*\\/g?i?)/',
				array(&$this, '_strToItems'), $content);
		...
		$content = preg_replace_callback('/¤@([^¤]+)¤/',
				array(&$this, '_itemsToStr'), $content);
		$content = preg_replace_callback('/¤#([^¤]+)¤/',
				array(&$this, '_decode'), $content); // Restore all \/, \", \'

...
class Moxiecode_ImageManagerPlugin extends Moxiecode_ManagerPlugin {
	...
	function onRPC(&$man, $cmd, $input) {
		...
		case "flipImage":
			...
			$filedata = array();
			$filedata["path"] = $file->getAbsolutePath();
			$filedata["vertical"] = isset($input["vertical"]) ? $input["vertical"]
					: false;
			$filedata["horizontal"] = isset($input["horizontal"]) ? $input["horizontal"]
					: false;
			$filedata["target"] = isset($input["target"]) ? $input["target"]
					: "";
			$filedata["temp"] = isset($input["temp"]) ? $input["temp"] : "";
		...
		case "saveImage":
			...
			if (checkBool($config["general.demo"])) {
				$result
						->add("FAILED", $man->encryptPath($input["target"]),
								"{#error.demo}");
			...
			if (isset($input["target"]) && $input["target"] != "") {
				$targetFile = $man
						->getFile(utf8_encode($file->getParent()),
								$input["target"]);
				$filedata["target"] = utf8_encode(
	...
	function onBeforeFileAction(&$man, $action, $file1, $file2) {
		if ($action == DELETE_ACTION) {
			...
			if (checkBool($config['filesystem.delete_format_images'])) {
				...
				$imageutils
						->deleteFormatImages($file1->getAbsolutePath(),
	...
	function onFileAction(&$man, $action, $file1, $file2) {
		...
		case ADD_ACTION:
			...
			if ($config["upload.format"]) {
				$imageutils = new $config['thumbnail'];
				$imageutils
						->formatImage($file1->getAbsolutePath(),
								$config["upload.format"],
								$config['upload.autoresize_jpeg_quality']);
			}
			...
			if (checkBool($config['upload.autoresize'])) {
				...
				$imageutils = new $config['thumbnail'];
				$percentage = min($config['upload.max_width'] / $fileWidth,
						$config['upload.max_height'] / $fileHeight);

				if ($percentage <= 1)
					$result = $imageutils
							->resizeImage($file1->getAbsolutePath(),
									$file1->getAbsolutePath(),
									round($fileWidth * $percentage),
									round($fileHeight * $percentage), $ext,
									$config['upload.autoresize_jpeg_quality']);
			}
			break;
		...
		case DELETE_ACTION:
			...
			if ($config['thumbnail.delete'] == true) {
				$thumbnailFolder = $man
						->getFile(
								dirname($file1->getAbsolutePath()) . "/"
										. $config['thumbnail.folder']);
				$thumbnailPath = $thumbnailFolder->getAbsolutePath() . "/"
						. $config['thumbnail.prefix']
	...
	function onCustomInfo(&$man, &$file, $type, &$input) {
		...
		if ($file->isFile()
				&& ($type == "list" || $type == "insert" || $type == "info")) {
			...
			if ($config['thumbnail.scale_mode'] == "percentage") {
				$percentage = min($config['thumbnail.width'] / $fileWidth,
						$config['thumbnail.height'] / $fileHeight);
			...
			if (($config["thumbnail.max_width"] != ""
					&& $fileWidth > $config["thumbnail.max_width"])
					|| ($config["thumbnail.max_height"] != ""
							&& $fileHeight > $config["thumbnail.max_height"]))
			...
			if ($type == "insert") {
				...
				if ($thumbFile->exists())
					$input["thumbnail_url"] = $man
							->convertPathToURI($thumbFile->getAbsolutePath(),
	...
	function _saveImage(&$man, &$file, &$filedata, &$result) {
		...
		if (strpos($file->getName(), "mcic_") !== 0)
			$tmpImage = "mcic_" . md5(session_id() . $file->getName()) . "."
					. $ext;
		else
		...
		if ($tempFile->exists()) {
			if ($filedata["target"] != "") {
				...
				$imageutils = new $config['thumbnail'];
				$imageutils
						->deleteFormatImages($targetfile->getAbsolutePath(),
								$config["upload.format"]);

				// Just ignore it if it's the same file
				if ($tempFile->getAbsolutePath()
						!= $targetfile->getAbsolutePath()) {
					...
					if ($config["edit.format"]) {
						$imageutils = new $config['thumbnail'];
						$imageutils
								->formatImage($targetfile->getAbsolutePath(),
										$config["edit.format"],
										$config['edit.jpeg_quality']);
					}
				}
	...
	function _cropImage(&$man, &$file, &$filedata, &$result) {
		...
		if ($filedata["temp"]) {
			...
			$tempFile = &$man
					->getFile(
							dirname($file->getAbsolutePath()) . "/" . $tmpImage);
			$tempFile->setTriggerEvents(false);

			$status = $imageutils
					->cropImage($file->getAbsolutePath(),
							$tempFile->getAbsolutePath(), $filedata["top"],
							$filedata["left"], $filedata["width"],
							$filedata["height"], $ext,
							$config["edit.jpeg_quality"]);
			if ($status) {
				$tempFile->importFile();
				$result
						->add("OK",
								$man->encryptPath($tempFile->getAbsolutePath()),
								"{#message.crop_success}");
			} else {
				$result
						->add("FAILED",
								$man->encryptPath($file->getAbsolutePath()),
			...
			$status = $imageutils
					->cropImage($file->getAbsolutePath(),
							$targetfile->getAbsolutePath(), $filedata["top"],
							$filedata["left"], $filedata["width"],
							$filedata["height"], $ext,
							$config["edit.jpeg_quality"]);
		...
		if ($filedata["temp"]) {
			...
			$status = $imageutils
					->resizeImage($file->getAbsolutePath(),
							$tempFile->getAbsolutePath(), $filedata["width"],
							$filedata["height"], $ext,
							$config["edit.jpeg_quality"]);
			if ($status) {
				$tempFile->importFile();
			...
			$status = $imageutils
					->resizeImage($file->getAbsolutePath(),
							$targetfile->getAbsolutePath(), $filedata["width"],
							$filedata["height"], $ext,
							$config["edit.jpeg_quality"]);
		...
		if ($filedata["temp"]) {
			...
			$status = $imageutils
					->rotateImage($file->getAbsolutePath(),
							$tempFile->getAbsolutePath(), $ext,
							$filedata["angle"], $config["edit.jpeg_quality"]);
			if ($status) {
				$tempFile->importFile();
				$result
						->add("OK",
								$man->encryptPath($tempFile->getAbsolutePath()),
								"{#message.rotate_success}");
			} else {
				$result
						->add("FAILED",
								$man->encryptPath($file->getAbsolutePath()),
			...
			$status = $imageutils
					->rotateImage($file->getAbsolutePath(),
							$targetfile->getAbsolutePath(), $ext,
							$filedata["angle"], $config["edit.jpeg_quality"]);
	...
	function _flipImage(&$man, &$file, &$filedata, &$result) {
		...
		if ($filedata["temp"]) {
			...
			$status = $imageutils
					->flipImage($file->getAbsolutePath(),
							$tempFile->getAbsolutePath(), $ext,
							$filedata["vertical"], $filedata["horizontal"]);
			if ($status) {
				$tempFile->importFile();
				$result
						->add("OK",
								$man->encryptPath($tempFile->getAbsolutePath()),
								"{#message.flip_success}");
			} else {
				$result
						->add("FAILED",
								$man->encryptPath($file->getAbsolutePath()),
	...
	function _cleanUp(&$man, $path) {
		...
		foreach ($files as $file) {
			if (strpos($file->getName(), "mcic_") === 0
					&& time() - $file->getLastModified() > 3600) {
	...
	function _getMediaInfo(&$man, $input) {
		...
		if (isset($input["url"])) {
			...
			if (!$man->verifyPath($input["path"]))
				trigger_error(
						sprintf(
								"Could not resolve URL: %s to a filesystem path. Could be that the image is outside the configured filesystem.rootpath.",
								$input["url"]), FATAL);
		}
		...
		$imagearray = array('gif', 'jpg', 'png', 'swf', 'psd', 'bmp', 'tiff',
		...
		$result = new Moxiecode_ResultSet(
				"name,path,url,size,type,created,modified,width,height,attribs,next,prev,custom");
		$custom = array();
		...
		$result
				->add(utf8_encode($file->getName()),
						$man->encryptPath($file->getAbsolutePath()),
						utf8_encode($url), $file->getLength(), $ext,
						date($config['filesystem.datefmt'],
								$file->getCreationDate()),
						date($config['filesystem.datefmt'],
								$file->getLastModified()), $width, $height,
						$attribs, $man->encryptPath($next),
						$man->encryptPath($prev), $custom);
	...
	function _createThumb(&$man, &$file) {
		...
		if (($config['thumbnail.use_exif'] == true)
				&& (function_exists("exif_thumbnail"))
				&& (strtolower($ext) == "jpg" || strtolower($ext) == "jpeg")) {
			$image = @exif_thumbnail($file->getAbsolutePath(), $exif_width,
					$exif_height, $exif_type);
		...
		if ((!$thumbnailFolder->exists())
				&& ($config['thumbnail.auto_generate'] == true))
		...
		if ($thumbnail->exists()) {
			...
			if (($thumbnailHeight != $targetHeight)
					|| ($thumbnailWidth != $targetWidth)) {
				$thumbnail->delete();
				$thumbnailResult = $imageutils
						->resizeImage($file->getAbsolutePath(),
								$thumbnail->getAbsolutePath(), $targetWidth,
								$targetHeight, $ext, $thumbnailQuality);
			}
	...
	function _streamThumb(&$man, $input) {
		...
		if ($thumbnail != false) {
			header(
					'Content-type: '
							. mapMimeTypeFromUrl($thumbnail->getName(),
			...
			header(
					'Content-type: '
							. mapMimeTypeFromUrl($file->getName(),
									"../" . $config['stream.mimefile']));

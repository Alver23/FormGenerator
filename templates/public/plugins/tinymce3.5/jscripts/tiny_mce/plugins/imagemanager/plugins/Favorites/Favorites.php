...
class Moxiecode_FavoritesPlugin extends Moxiecode_ManagerPlugin {
	...
	function _removeFavorites(&$man, $input) {
		...
		for ($i = 0; isset($input['path' . $i]); $i++) {
			...
			if ($status)
				$result
						->add("OK", $man->encryptPath($input["path" . $i]),
								"Path was removed.");
			else
				$result
						->add("FAILED", $man->encryptPath($input["path" . $i]),
	...
	function addFavorite(&$man, $path) {
		...
		$maxfavorites = isset($config["favorites.max"]) ? $config["favorites.max"]
				: $this->_maxfavorites;
	...
	function setCookieData($type, $val) {
		...
		setcookie("MCManagerFavoriteCookie_" . $type, $val, time()
				+ (3600 * 24 * 30)); // 30 days
	}

	function clearFavorites(&$man) {
		setcookie("MCManagerFavoriteCookie_" . $man->getType(), "",
...
class FavoriteFile extends Moxiecode_BaseFileImpl {
	function FavoriteFile(&$manager, $absolute_path, $child_name = "",
			$type = MC_IS_FILE) {
		$absolute_path = str_replace('favorite://', '', $absolute_path);
		Moxiecode_BaseFileImpl::Moxiecode_BaseFileImpl($manager,
				$absolute_path, $child_name, $type);
	}
	...
	function _setCookieData($type, $val) {
		setcookie("MCManagerFavoriteCookie_" . $type, $val,

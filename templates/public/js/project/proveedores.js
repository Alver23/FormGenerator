...
var content = $("#idForm"), button = $("#newform"), table = $("#divtable"); 
	if (content.is(':hidden')) { 
	 ...
	 var tProveedores = $('#table').dataTable({ 
	  ...
	  $(document).on('click', '.delete', function(){ 
		...
		$.post('proveedores', { act: 'delete', m: cod }, function (obj) {	
			tProveedores.fnClearTable(true); 
				$('body').showMessage({ 
	  ...
	  $(document).on('click', '.edit', function(){ 
		var cod = $(this).attr('rel'); 
		$.post('proveedores', { act: 'edit', m: cod }, function (obj) {	
		}, 'json'); 
	  }); 

 
	var v = $('#proveedores').validate({ 
			rules : { 

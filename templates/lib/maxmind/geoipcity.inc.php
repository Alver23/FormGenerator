...
function getrecordwithdnsservice($str) {
	...
	if ($record->region != "") {
		if (($record->country_code == "US") || ($record->country_code == "CA")) {
			$record->regionname = $GLOBALS['ISO'][$record->country_code][$record
					->region];
		} else {
			$record->regionname = $GLOBALS['FIPS'][$record->country_code][$record
					->region];
		}
	}
	return $record;
...
function _get_record($gi, $ipnum) {
	...
	if ($gi->flags & GEOIP_MEMORY_CACHE) {
		$record_buf = substr($gi->memory_buffer, $record_pointer,
				FULL_RECORD_LENGTH);
	} elseif ($gi->flags & GEOIP_SHARED_MEMORY) {
		$record_buf = @shmop_read($gi->shmid, $record_pointer,
				FULL_RECORD_LENGTH);
	} else {
	...
	if (GEOIP_CITY_EDITION_REV1 == $gi->databaseType) {
		...
		if ($record->country_code == "US") {
			...
			$record->metro_code = $record->dma_code = floor(
					$metroarea_combo / 1000);

...
class conNeg {
	 ...
	 */
	static private function genericNeg($field, $returnType, $rawAppTypes = null,
			$productVals = true, $mimeNeg = false) {
		...
		// Parse application provided string as field.
			...
			list($appTypes, $appAcceptExtens) = self::parseField($rawAppTypes,
					$mimeNeg, false);
		...
		for ($i = 0; $i < count($userTypes['type']); $i++) {
			...
			if (!$appVals) {
				if ($mimeNeg
						&& ($userTypes['mimeType'][$i] == '*'
								|| ($userTypes['mimeSubtype'][$i] == '*'))
			...
			// (match > mime subtype wildcard match > wildcard match)
 ...
 else if ($appTypes['acceptExtens'][$key]
								&& $appTypes[$appTypes['acceptExtens'][$key]][$key]
										== $appTypes[$appTypes['acceptExtens'][$key]][$key]) {
							$appTypes['specificness'][$key] = self::WILDCARD_NONE;
							$appTypes['qFactorUser'][$key] = $userTypes['qFactorUser'][$i];
				...
				// a type wildcard.
 else if ($mimeNeg && $userTypes['mimeType'][$i] == '*'
						&& $userTypes['mimeSubtype'][$i] == '*'
						|| (!$mimeNeg && $userTypes['type'][$i] == '*')) {
		...
		if ($productVals && $appVals) {
			for ($i = 0; $i < count($appTypes['type']); $i++) {
				$appTypes['qFactorProduct'][$i] = $appTypes['qFactorUser'][$i]
						* $appTypes['qFactorApp'][$i];
			}
		}
	...
	static protected function parseField($field, $mimeNeg, $uaField = true) {
		...
		if ($mimeNeg) {
			if (!preg_match_all(
					'/([a-z\-\+\*]+)\/([a-z\-\+\*]+)\s*;?\s*(:?(?:q=(0\.\d{1,5}|1\.0|[01])\s*?;?\s*?)|(?:([a-z]+)=(")?([0-9a-z\-\+\.]+)\s*?;?\s*?)\\6?){0,},*/i',
					$field, $matches)) {
			...
			for ($i = 0; $i < count($matches[0]); $i++) {
				if (strlen($matches[5][$i])
						&& !array_key_exists($matches[5][$i], $extensArr)) {
		...
		// Charset, Language and Encoding can be handled together
			...
			if (!preg_match_all(
					'/([a-z\-0-9\*]+)\s*;?\s*q?=?(0\.\d{1,5}|1\.0|[01])?,*/i',
					$field, $matches)) {
				return array(false, array());
			}
	 ...
	 */
	static private function sortTypes(&$types, $productVals, $appVals,
			$appAcceptExtens) {
	...
	static public function mimeBest($appTypes = null, $productVals = true) {
		if (isset($_SERVER['HTTP_ACCEPT'])) {
			return self::genericNeg($_SERVER['HTTP_ACCEPT'], 'best', $appTypes,
					$productVals, true);
		} else {
			return false;
		}
	...
	static public function mimeAll($appTypes = null, $productVals = true) {
		if (isset($_SERVER['HTTP_ACCEPT'])) {
			return self::genericNeg($_SERVER['HTTP_ACCEPT'], 'all', $appTypes,
					$productVals, true);
		} else {
			return false;
		}
	...
	static public function charBest($appTypes = null, $productVals = true) {
		if (isset($_SERVER['HTTP_ACCEPT_CHARSET'])) {
			return self::genericNeg($_SERVER['HTTP_ACCEPT_CHARSET'], 'best',
					$appTypes, $productVals);
		} else {
			return false;
		}
	}
	...
	static public function charAll($appTypes = null, $productVals = true) {
		if (isset($_SERVER['HTTP_ACCEPT_CHARSET'])) {
			return self::genericNeg($_SERVER['HTTP_ACCEPT_CHARSET'], 'all',
					$appTypes, $productVals);
		} else {
			return false;
		}
	...
	static public function encBest($appTypes = null, $productVals = true) {
		if (isset($_SERVER['HTTP_ACCEPT_ENCODING'])) {
			return self::genericNeg($_SERVER['HTTP_ACCEPT_ENCODING'], 'best',
					$appTypes, $productVals);
		} else {
			return false;
		}
	...
	static public function encAll($appTypes = null, $productVals = true) {
		if (isset($_SERVER['HTTP_ACCEPT_ENCODING'])) {
			return self::genericNeg($_SERVER['HTTP_ACCEPT_ENCODING'], 'all',
					$appTypes, $productVals);
		} else {
			return false;
		}
	...
	static public function langBest($appTypes = null, $productVals = true) {
		if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
			return self::genericNeg($_SERVER['HTTP_ACCEPT_LANGUAGE'], 'best',
					$appTypes, $productVals);
		} else {
			return false;
		}
	...
	static public function langAll($appTypes = null, $productVals = true) {
		if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
			return self::genericNeg($_SERVER['HTTP_ACCEPT_LANGUAGE'], 'all',
					$appTypes, $productVals);
		} else {
			return false;
		}

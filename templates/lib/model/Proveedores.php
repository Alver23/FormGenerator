...
class Proveedores{
	...
	public function exits(){
		return (count($this->listar(array("idproveedores" => $this->getId())))===1);
	}	
	
	//Guarda o actualiza el objeto en la base de datos, la accion se determina por la clave primaria
	public function save(){
		if(empty($this->idproveedores) || !$this->exits()){
			$this->setCommonValuesInsert();
			$this->idproveedores = $this->con->autoInsert(array(
		...
		"created_at" => $this->createdAt,
		"updated_at" => $this->updatedAt,
		),"proveedores",$this->getId());
	}
    
	public function cargarPorId($value,$campowhere = "idproveedores",$camp = array()){
		if(!empty($value)){
			...
			$sql = ("SELECT {$fieldslistar} FROM proveedores WHERE {$campowhere} = %s");
	...
	public function listar($filtros = array(), $orderBy = '', $limit = "0,30", $exactMatch = false, $fields = '*', $idInKeys = true){
		$whereA = array();
		if(!$exactMatch){
			$campos = $this->con->query("DESCRIBE proveedores");
			$listicos = array();
		...
		$rows =$this->con->query("SELECT $fields,idproveedores FROM `proveedores`  WHERE $where $orderBy LIMIT $limit");
		$rowsI = array();
		foreach($rows as $row){
        	if($idInKeys)
				$rowsI[$row["idproveedores"]] = $row;
            else
            	$rowsI[] = $row;
		}
		return $rowsI;
	...
	function listarObj($filtros = array(), $orderBy = '', $limit = "0,30", $exactMatch = false, $fields = '*'){
		...
		foreach($rows as $row){
			$obj = clone $this;
			$obj->cargarPorId($row["idproveedores"]);
			$rowsr[$row["idproveedores"]] = $obj;
		}
		return $rowsr;
	}
	public function delete($value = null,$key = "idproveedores"){
		if(!empty($value)){
			$this->setIdproveedores($value);
		}
		$value = $this->getId();
		return $this->con->autoDelete("proveedores",$key,$value);
	}
}
?>
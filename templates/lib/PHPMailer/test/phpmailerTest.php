...
class phpmailerTest extends PHPUnit_Framework_TestCase {
	...
	function setUp() {
		...
		if (array_key_exists('mail_cc', $_REQUEST)
				and strlen($_REQUEST['mail_cc']) > 0) {
	...
	function test_Embedded_Image() {
		...
		if (!$this->Mail
				->AddEmbeddedImage("test.png", "my-attach", "test.png",
						"base64", "image/png")) {
			$this->assertTrue(false, $this->Mail->ErrorInfo);
			return;
		}
	...
	function test_Addressing() {
		$this
				->assertFalse($this->Mail->AddAddress('a@example..com'),
						'Invalid address accepted');
		$this
				->assertTrue($this->Mail->AddAddress('a@example.com'),
						'Addressing failed');
		$this
				->assertFalse($this->Mail->AddAddress('a@example.com'),
						'Duplicate addressing failed');
		$this
				->assertTrue($this->Mail->AddCC('b@example.com'),
						'CC addressing failed');
		$this
				->assertFalse($this->Mail->AddCC('b@example.com'),
						'CC duplicate addressing failed');
		$this
				->assertFalse($this->Mail->AddCC('a@example.com'),
						'CC duplicate addressing failed (2)');
		$this
				->assertTrue($this->Mail->AddBCC('c@example.com'),
						'BCC addressing failed');
		$this
				->assertFalse($this->Mail->AddBCC('c@example.com'),
						'BCC duplicate addressing failed');
		$this
				->assertFalse($this->Mail->AddBCC('a@example.com'),
						'BCC duplicate addressing failed (2)');
		$this
				->assertTrue($this->Mail->AddReplyTo('a@example.com'),
						'Replyto Addressing failed');
		$this
				->assertFalse($this->Mail->AddReplyTo('a@example..com'),
	...
	function test_Translations() {
		...
		foreach (new DirectoryIterator('../language') as $fileInfo) {
			...
			if (preg_match('/^phpmailer\.lang-([a-z_]{2,})\.php$/',
				...
				$this
						->assertTrue(empty($missing),
								"Missing translations in $lang: "
										. implode(', ', $missing));
				$this
						->assertTrue(empty($extra),
	...
	function test_Miscellaneous() {
		...
		$this
				->assertFalse($this->Mail->set('x', 'y'),
						'Invalid property set succeeded');
		$this
				->assertTrue($this->Mail->set('Timeout', 11),

...
class sfTemplateEngine {
	protected $loader = null, $renderers = array(), $current = null, $helperSet = array(), $parents = array(), $stack = array(), $slots = array(), $openSlots = array(), $charset = 'UTF-8';
	 ...
	 */
	public function __construct(sfTemplateLoaderInterface $loader,
			array $renderers = array(), sfTemplateHelperSet $helperSet = null) {
		$this->loader = $loader;
		$this->renderers = $renderers;

		$this
				->setHelperSet(
						null === $helperSet ? new sfTemplateHelperSet()
								: $helperSet);
	...
	public function render($name, array $parameters = array()) {
		...
		if (false === $template) {
			throw new InvalidArgumentException(
					sprintf(
							'The template "%s" does not exist (renderer: %s).',
							$name, $renderer));
		}
		...
		if (!isset($this->renderers[$renderer])) {
			throw new InvalidArgumentException(
					sprintf('The renderer "%s" is not registered.', $renderer));
		}

		// render
		if (false
				=== $content = $this->renderers[$renderer]
						->evaluate($template, $parameters)) {
			throw new RuntimeException(
					sprintf(
							'The template "%s" cannot be rendered (renderer: %s).',
							$name, $renderer));
		}
	...
	public function escape($value) {
		return is_string($value)
				|| (is_object($value) && method_exists($value, '__toString')) ? htmlspecialchars(
						$value, ENT_QUOTES, $this->charset) : $value;
	}

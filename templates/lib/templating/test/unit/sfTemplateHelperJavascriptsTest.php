...
$helperSet = new sfTemplateHelperSet(array(new sfTemplateHelperAssets(),));
...
$helper->add('/foo');
$t
		->is($helper->get(), array('/foo' => array()),
				'->add() does not add the same JavaScript twice');
$helper = new sfTemplateHelperJavascripts();
$helperSet->set($helper);
$helperSet->get('assets')->setBaseURLs('http://assets.example.com/');
$helper->add('foo');
$t
		->is($helper->get(), array('http://assets.example.com/foo' => array()),
				'->add() converts the JavaScript to a public path');
...
$helperSet
		->setEngine(
				$engine = new sfTemplateEngine(
						new sfTemplateLoaderFilesystem('/')));
$helper->add('foo', array('class' => 'ba>'));
$t
		->is($helper->__toString(),
				'<script type="text/javascript" src="/foo" class="ba&gt;"></script>',

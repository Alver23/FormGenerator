...
$loader = new sfTemplateLoaderFilesystem(
		array(dirname(__FILE__) . '/fixtures/templates/%name%.%renderer%'));
...
$t
		->ok($renderer->getEngine() === $engine,
				'->setEngine() sets the engine instance tied to this renderer');
...
$renderer->setEngine($engine);
$t
		->is($renderer->get('foo'), 'bar',
				'__call() proxies to the embedded engine instance');
...
$renderer->setEngine($engine);
$t
		->is((string) $renderer->bar, 'foo',
				'__get() proxies to the embedded engine instance');

...
$helperSet = new sfTemplateHelperSet(
		array('foo' => $helper = new SimpleHelper('foo')));
$t
		->ok($helperSet->has('foo'),
				'__construct() takes an array of helpers as its first argument');
...
$helperSet = new sfTemplateHelperSet(
		array('foo' => $helper = new SimpleHelper('foo')));
$t
		->ok($helper->getHelperSet() === $helperSet,
				'->__construct() changes the embedded helper set of the given helpers');
...
$t
		->ok($helper->getHelperSet() === $helperSet,
				'->set() changes the embedded helper set of the helper');
...
$t
		->ok(!$helperSet->has('bar'),
				'->has() returns false if the helper is not defined');
